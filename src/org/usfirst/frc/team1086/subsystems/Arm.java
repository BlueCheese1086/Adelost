...
public class Arm implements Tickable {
		...
		if (!inputManager.getPixy()) {
			if (inputManager.manualArm() != 0)
				...
				armMotor.set(ControlMode.MotionMagic,
						(1 - armPos / Constants.MAX_ARM_ANGLE) * Constants.MAX_ARM_ENC_UNITS);
			}
		}
	...
	public double getArmPosition() {
		return Constants.MAX_ARM_ANGLE
				* (1 - (double) armMotor.getSelectedSensorPosition(0) / Constants.MAX_ARM_ENC_UNITS);
	}
}